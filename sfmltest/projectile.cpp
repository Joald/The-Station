...
void projectile::setorig(CMob *porig)
{
    this->orig=porig;
...
bool projectile::canWalk(tile floortile[n][m],Station & game)
{
    int ox=this->x;
    int oy=this->y;
    for(int i=0;i<n;i++)
    {
        for(int j=0;j<m;j++)
        {
            if(!floortile[i][j].walkable && ox>floortile[i][j].x && ox<floortile[i][j].x+100 && oy>floortile[i][j].y && oy<floortile[i][j].y+104)
                return false;
        }
    }
    int fixw=170;
...
if(game.isUnsized)
{
    ...
    if(this->y>game.h-fixh+fixh*2/5 || this->x>game.w-fixw-fixw/20 || this->y<fixh || this->x<fixw)
        return false;
    else
        return true;
