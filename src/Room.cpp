...
Room::Room(Station&game)
{
    ...
    this->dropst.loadFromFile("items_small.png");
    this->dbottom=true;
    this->dtop=true;
    this->dleft=true;
    this->dright=true;
    ...
    for(int i=0; i<n; i++)
    {
        for(int j=0; j<m; j++)
        {
            ...
            los=rand()%100;
            if(!this->floortile[i-1][j].walkable || !this->floortile[i][j-1].walkable || !this->floortile[i][j+1].walkable || !this->floortile[i+1][j].walkable)
                los/=2;
            if(((i==7 || i==8)&&(j==0 || j==m-1))||((i==0 || i==n-1)&& j==2)||((i==7 || i==8)&& j==2))
                los=100;
            ...
            if(i || j)
            {
                if(!i)
                {
                    this->floortile[i][j].sprite.setPosition(fixw*w/1920,this->floortile[i][j-1].y+this->floort.getSize().y);
                ...
                if(!j)
                {
                    this->floortile[i][j].sprite.setPosition(this->floortile[i-1][j].x+this->floort.getSize().x,fixh*h/1080);
                ...
                {
                    this->floortile[i][j].sprite.setPosition(this->floortile[i-1][j].x+this->floort.getSize().x,this->floortile[i][j-1].y+this->floort.getSize().y);
                    this->floortile[i][j].x=this->floortile[i-1][j].x+this->floort.getSize().x;
                    this->floortile[i][j].y=this->floortile[i][j-1].y+this->floort.getSize().y;
                }
            }
...
/*
void Room::changeTo (const Room& ox,tile floortil[n][m])
{
    this->floort.loadFromFile("floor_tile.png");
    this->dtopt.loadFromFile("doorup.png");
    this->dbottomt.loadFromFile("doorbot.png");
    this->dleftt.loadFromFile("doorleft.png");
    this->drightt.loadFromFile("doorright.png");
    this->enemyt.loadFromFile("enemy.png");
    this->chestt.loadFromFile("chest.png");
    this->sht.loadFromFile("shot.png");
