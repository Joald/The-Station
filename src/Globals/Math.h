...
static constexpr float distanceFromLineNumerator(
        const sf::Vector2f& point, const sf::Vector2f& linePoint1,
        const sf::Vector2f& linePoint2) {
...
constexpr float distanceFromLine(
        const sf::Vector2f& point,
        const sf::Vector2f& linePoint1,
        const sf::Vector2f& linePoint2,
        float distance12
) {
    return Private::distanceFromLineNumerator(point, linePoint1, linePoint2) / distance12;
}

constexpr float distanceFromLine(const sf::Vector2f& point, const sf::Vector2f& linePoint1,
                                 const sf::Vector2f& linePoint2) {
    return distanceFromLine(point, linePoint1, linePoint2, distance(linePoint1, linePoint2));
}

constexpr float distanceFromLineNoSqrt(
        const sf::Vector2f& point,
        const sf::Vector2f& linePoint1,
        const sf::Vector2f& linePoint2,
        float sqDistance12
) {
    return Private::distanceFromLineNumerator(point, linePoint1, linePoint2) / sqDistance12;
}

constexpr float distanceFromLineNoSqrt(const sf::Vector2f& point, const sf::Vector2f& linePoint1,
                                       const sf::Vector2f& linePoint2) {
    return distanceFromLineNoSqrt(point, linePoint1, linePoint2,
                                  sqDistance(linePoint1, linePoint2));
}

constexpr float distanceFromLineSegment(const sf::Vector2f& point, const sf::Vector2f& linePoint1,
                                        const sf::Vector2f& linePoint2) {
    ...
    const float t = std::max(0.f, std::min(1.f,
                                           dotProduct(point - linePoint1, linePoint2 - linePoint1) /
                                           sqDist));
